...
import { ClipboardIcon, ThumbsDownIcon, ThumbsUpIcon, InfoIcon } from '../icons';
...
export const InfoButton = (props: RatingButtonProps) => {
  return (
    ...
    >
      <Show when={!props.isLoading} fallback={<Spinner class="text-white" />}>
        <InfoIcon color={props.sendButtonColor} class={'send-icon flex ' + (props.disableIcon ? 'hidden' : '')} />
      </Show>
    </button>
  );
};
